...
def approve_review(review_id: int, officer_id: int):
    ...
    try:
        ...
        if not row:
            raise HTTPException(
                status_code=404, detail="Review Record Haikupatikana!")
        ...
        conn.commit()
        return {"status": "success", "message": "Transaction cleared and marked as NOT FRAUD"}
...
def reject_review(review_id: int, officer_id: int):
    ...
    try:
        ...
        """, (officer_id, review_id))
        conn.commit()
        return {"status": "success", "message": "Transaction confirmed as FRAUD and blocked successfully"}
    except Exception as e:
        conn.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
